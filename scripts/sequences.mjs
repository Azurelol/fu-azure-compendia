...
function isTooClose(source, target) {
  ...
  return target.bounds
    .pad(gridSize * 0.5, gridSize * 0.5)
    .intersects(source.bounds);
}
...
function playSoundEffect(sequence, preset) {
  ...
  if (
    !AzureCompendiaSettings.isEnabled(AzureCompendiaSettings.keys.playSounds)
  ) {
    return;
  }
...
function dodgeTarget(sequence, source, target) {
  if (
    !AzureCompendiaSettings.isEnabled(
      AzureCompendiaSettings.keys.animateHitReaction,
    )
  ) {
    return;
  }
...
function shakeTarget(sequence, source, target) {
  ...
  const values = {
    x: [
      0,
      -amplitude * shakeDirection.y,
      amplitude * shakeDirection.y,
      (-amplitude * shakeDirection.y) / 4,
      (amplitude * shakeDirection.y) / 4,
      0,
    ],
    y: [
      0,
      amplitude * shakeDirection.x,
      -amplitude * shakeDirection.x,
      (amplitude * shakeDirection.x) / 4,
      (-amplitude * shakeDirection.x) / 4,
      0,
    ],
  };
  ...
  sequence
    ...
    .scaleToObject(
      Math.max(target.document.texture.scaleX, target.document.texture.scaleX),
      {
        uniform: false,
...
function animateSkill(sequence, sourceToken, item, traits) {
  ...
  if (skill) {
    ...
    const section = playAnimationOnToken(
      sequence,
      skill,
      sourceToken,
      skillScale,
...
function animateCheck(sequence, miss, traits, token) {
  if (
    !AzureCompendiaSettings.isEnabled(AzureCompendiaSettings.keys.animateCheck)
  ) {
    return;
  }
  if (miss) {
...
function animateMeleeDash(sequence, item, traits, type, sourceToken, targets) {
  ...
  for (const target of targets) {
    let hitPosition = Ray.towardsPoint(
      target.token.center,
      sourceToken.center,
      gridSize,
    ).B;
    ...
    let dash = sequence
      ...
      .scaleToObject(
        Math.max(
          sourceToken.document.texture.scaleX,
          sourceToken.document.texture.scaleY,
        ),
        {
          uniform: false,
          considerTokenScale: true,
  ...
  sequence.animation().on(sourceToken).fadeIn(500);
}
...
function animateDamageTaken(
  ...
  if (!miss) {
    playAnimationOnToken(
      sequence,
      AzureCompendiaPresets.get(type),
      targetToken,
    );
...
function animateSpellAttack(
  ...
  const preset = AzureCompendiaPresets.resolveSpellAttack(
    item,
    type,
    multiple,
    traits,
  ...
  if (multiple && preset.aoe) {
    ...
    const spellSequence = sequence
      .effect()
      .file(spell.animation)
      .atLocation({
        x: maxTargetX - targetWidth / 2,
        y: maxTargetY - targetHeight / 2,
      })
...
function animateItem(sequence, item, type, actor, token, targets) {
  ...
  if (targets.length > 0) {
    ...
    for (const target of targets) {
      ...
      playAnimationOnToken(
        sequence,
        AzureCompendiaPresets.get("splash"),
        target.token,
      );
    }
  }
